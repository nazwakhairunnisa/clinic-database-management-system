...
<div class="font-['Roboto',sans-serif] px-4 sm:px-6 lg:px-10 py-6">
    ...
    <div class="flex items-center justify-between mb-6 gap-4">
        ...
        {{-- Filter Bar (Kiri) --}}
        <form method="GET" action="{{ route('owner.laporan_penjualan.index') }}" id="filterForm" class="flex-1">
        ...
        <table class="w-full min-w-[1200px] text-sm border-collapse">
            ...
            <tbody class="text-gray-700">
                ...
                <tr class="border-b hover:bg-gray-50">
                    ...
                    @foreach($monthlyData['months'] as $item)
                    <td class="px-4 py-3 text-center">
                        Rp {{ number_format($item['expense_pembelian_obat'] ?? 0, 0, ',', '.') }}
                    </td>
                    @endforeach
                    <td class="px-4 py-3 text-center font-semibold bg-gray-50">
                        Rp {{ number_format($monthlyData['totals']['expense_pembelian_obat'] ?? 0, 0, ',', '.') }}
                    </td>
                </tr>
                ...
                <tr class="border-b hover:bg-gray-50">
                    ...
                    @foreach($monthlyData['months'] as $item)
                    <td class="px-4 py-3 text-center">
                        Rp {{ number_format($item['expense_operasional'] ?? 0, 0, ',', '.') }}
                    </td>
                    @endforeach
                    <td class="px-4 py-3 text-center font-semibold bg-gray-50">
                        Rp {{ number_format($monthlyData['totals']['expense_operasional'] ?? 0, 0, ',', '.') }}
                    </td>
                </tr>
    ...
    <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl shadow p-5 border border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                ...
                Detail Pemasukan - {{ \Carbon\Carbon::create($tahun, $bulan, 1)->translatedFormat('F Y') }}
            </h3>
        ...
        <div class="bg-white rounded-xl shadow p-5 border border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                ...
                Detail Pengeluaran - {{ \Carbon\Carbon::create($tahun, $bulan, 1)->translatedFormat('F Y') }}
            </h3>
