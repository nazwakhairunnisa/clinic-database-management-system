...
<div class="font-['Roboto',sans-serif] bg-[#F8F6F1] min-h-screen flex justify-center items-start pt-10 sm:pt-14">
    <div class="bg-white rounded-2xl shadow-lg w-full max-w-3xl p-6 sm:p-8 transition-all duration-300">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('owner.stok-obat.index') }}" class="text-gray-400 hover:text-gray-600 transition">
        ...
        {{-- FORM --}}
        <form action="{{ route('owner.stok-obat.update', $obat->id_obat) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="text-sm font-medium text-gray-600">Stok Terkini</label>
                    <input type="text" value="{{ number_format($obat->stok_terkini, 0, ',', '.') }} {{ $obat->satuan }}" readonly
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-100 text-gray-600">
                    <p class="text-xs text-gray-500 mt-1">Stok tidak bisa diubah langsung, gunakan menu Pembelian Obat</p>
                </div>
            </div>
            ...
            <div class="pt-2 flex space-x-3">
                ...
                <a href="{{ route('owner.stok-obat.index') }}"
