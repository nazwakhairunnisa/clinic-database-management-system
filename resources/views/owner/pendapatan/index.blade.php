...
<div class="font-['Roboto',sans-serif]">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 pt-6">
        ...
        <div class="flex w-full items-center justify-between gap-3">
            ...
            {{-- Search Bar --}}
            <form method="GET" action="{{ route('owner.pendapatan.index') }}" class="relative flex-grow">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search Pendapatan"
            ...
            {{-- Export PDF --}}
            <a href="{{ route('owner.pendapatan.export') . '?' . http_build_query(request()->query()) }}"
        ...
        <table class="min-w-[800px] w-full text-sm text-left border-collapse">
            ...
            <tbody class="text-gray-700">
                ...
                <tr class="bg-white hover:bg-gray-50 transition-all duration-200 border-b border-gray-200">
                    ...
                    <td class="px-4 py-3">
                    @php
                        $metode = $row->metode_pembayaran ?? 'cash'; // fallback kalau null
                    @endphp
                    @if($metode === 'cash')
                        <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs">Cash</span>
                    ...
                    <td class="px-4 py-3 flex space-x-3">
                        @if(!$row->id_pembayaran)
                            {{-- EDIT --}}
                            <a href="{{ route('owner.pendapatan.edit', $row->id_transaksi) }}"
                            ...
                            {{-- DELETE --}}
                            <form action="{{ route('owner.pendapatan.destroy', $row->id_transaksi) }}" 
                                  method="POST" 
                        ...
                        @else
                            <span class="text-gray-400 text-xs">Dari Reservasi</span>
                        @endif
                    </td>
                </tr>
                @empty
