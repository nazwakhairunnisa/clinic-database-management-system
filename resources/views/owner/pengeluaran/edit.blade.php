...
<div class="font-['Roboto',sans-serif] bg-[#F8F6F1] min-h-screen flex justify-center items-start pt-10 sm:pt-14">
    ...
    <div class="bg-white rounded-2xl shadow-lg w-full max-w-3xl p-6 sm:p-8 transition-all duration-300 relative">
        ...
        {{-- FORM --}}
        <form action="{{ route('owner.pengeluaran.update', $pengeluaran->id_transaksi) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" 
                        name="nama_transaksi"
                        value="{{ old('nama_transaksi', $pengeluaran->nama_transaksi) }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" 
                        ...
                        value="{{ old('tanggal_transaksi', $pengeluaran->tanggal_transaksi->format('Y-m-d')) }}"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm
                ...
                <div>
                    ...
                    <select 
                        ...
                        <option value="cash" {{ old('metode_pembayaran', $pengeluaran->metode_pembayaran) == 'cash' ? 'selected' : '' }}>Cash</option>
                        <option value="transfer" {{ old('metode_pembayaran', $pengeluaran->metode_pembayaran) == 'transfer' ? 'selected' : '' }}>Transfer</option>
                        <option value="ewallet" {{ old('metode_pembayaran', $pengeluaran->metode_pembayaran) == 'ewallet' ? 'selected' : '' }}>E-Wallet</option>
                    </select>
            ...
            <div>
                ...
                <textarea 
                    ...
                    class="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm
                           focus:ring-2 focus:ring-[#EED892] outline-none">{{ old('keterangan', $pengeluaran->keterangan) }}</textarea>
            </div>
