...
@section('pageTitle', 'Edit Pembelian Obat')
...
<div class="font-['Roboto',sans-serif] bg-[#F8F6F1] min-h-screen flex justify-center items-start pt-10 sm:pt-14">
    <div class="bg-white rounded-2xl shadow-lg w-full max-w-3xl p-6 sm:p-8 transition-all duration-300">
        ...
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-lg sm:text-xl font-semibold text-black">Edit Pembelian Obat</h2>
            <a href="{{ route('owner.pembelian_obat.index') }}" class="text-gray-400 hover:text-gray-600 transition">
        ...
        {{-- FORM --}}
        <form action="{{ route('owner.pembelian_obat.update', $pembelian->id_pembelian_obat) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="id_obat" id="id_obat" required
                        ...
                        <option value="{{ $obat->id_obat }}" 
                            {{ old('id_obat', $pembelian->id_obat) == $obat->id_obat ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="status_pembayaran" id="status_pembayaran" required 
                        ...
                        <option value="lunas" 
                            {{ old('status_pembayaran', $pembelian->status_pembayaran) == 'lunas' ? 'selected' : '' }}>
                            Sudah Dibayar
                        </option>
                        <option value="belum" 
                            {{ old('status_pembayaran', $pembelian->status_pembayaran) == 'belum' ? 'selected' : '' }}>
                            Belum Dibayar
                        </option>
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-100 text-gray-700 font-semibold" 
                        id="totalHarga">
                ...
                <div>
                    ...
                    <div id="metode-pembayaran-container">
                        ...
                        @php
                            // Ambil metode pembayaran dari transaksi keuangan jika ada
                            $metodePembayaran = $pembelian->transaksiKeuangan->metode_pembayaran ?? old('metode_pembayaran', '');
                        @endphp
                        <select name="metode_pembayaran" id="metode_pembayaran"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#EED892] outline-none">
                            <option value="">- Pilih Metode -</option>
                            <option value="cash" {{ $metodePembayaran == 'cash' ? 'selected' : '' }}>Cash</option>
                            <option value="transfer" {{ $metodePembayaran == 'transfer' ? 'selected' : '' }}>Transfer Bank</option>
                            <option value="ewallet" {{ $metodePembayaran == 'ewallet' ? 'selected' : '' }}>E-Wallet</option>
                        </select>
            ...
            <div class="pt-2 flex space-x-3">
                ...
                <a href="{{ route('owner.pembelian_obat.index') }}"
...
function toggleFields() {
    ...
    if (status === 'belum') {
        ...
        // Sudah Dibayar: Tampilkan Metode Pembayaran, Sembunyikan Jatuh Tempo
        ...
        metodePembayaranInput.required = true;
