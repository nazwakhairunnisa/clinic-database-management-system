...
<div class="font-['Roboto',sans-serif]">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 pt-6">
        <div class="flex w-full items-center justify-between gap-3">
            ...
            {{-- Search Bar --}}
            <form method="GET" action="{{ route('owner.pembelian_obat') }}" class="relative flex-grow flex gap-2">
...
function exportToExcel() {
    let table = document.getElementById('pembelianObatTable');
    let html = table.outerHTML;
    let url = 'data:application/vnd.ms-excel,' + encodeURIComponent(html);
    let downloadLink = document.createElement("a");
    document.body.appendChild(downloadLink);
    downloadLink.href = url;
    downloadLink.download = 'pembelian_obat_' + new Date().getTime() + '.xls';
    downloadLink.click();
